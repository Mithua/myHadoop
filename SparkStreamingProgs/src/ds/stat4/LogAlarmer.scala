...
object LogAlarmer {
  ...
  def main(args: Array[String]) {
    ...
    // Tally up statuses over a 2-minute window sliding every second
    val statusCounts = successFailure.countByValueAndWindow(Seconds(120), Seconds(1))
    ...
    statusCounts.foreachRDD((rdd, time) => {
      ...
      println("Total success: " + totalSuccess + "\nTotal failure: " + totalError + "\nTotal other: " + totalOther + "\n")
      ...
      if (totalError + totalSuccess + totalOther > 100) {
        ...
        val ratio:Double = util.Try(totalError.toDouble / (totalSuccess.toDouble) + totalError.toDouble + totalOther.toDouble) getOrElse 1.0
        ...
        if (ratio > 0.15) {
          ...
          println("Wake somebody up! Something is going wrong.")
        } else {
          println("All systems okay.")
        }
      }
    })
