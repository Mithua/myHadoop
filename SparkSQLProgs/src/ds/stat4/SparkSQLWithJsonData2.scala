...
def main(args: Array[String]) {
	...
	val artistDataFrame = spark.read.json("../artists_en.json")
	val artistDataSet = artistDataFrame.as[Artist]

			println("Here is our inferred schema:")		
			artistDataSet.printSchema()

			println("Count of no. of records:")
			println(artistDataSet.count()) // n = 163
			
			println("Let's select all rows : ")
			artistDataSet.show(163)
