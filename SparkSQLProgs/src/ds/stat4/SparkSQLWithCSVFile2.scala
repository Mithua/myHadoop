...
case class Cust (custId:BigInt, firstName:String, lastName:String, age:BigInt)
...
def mapper1(line:String): Cust = {
		val fields = line.split(',')
				val cust:Cust = Cust(fields(0).toInt, fields(1), fields(2), fields(3).toInt)
				return cust
}
...
def main(args: Array[String]) {
	...
	println("Let's select the Count no of customers by age:")
	cust.groupBy("age").count().show()
